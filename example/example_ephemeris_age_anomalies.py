#!/usr/bin/env python3
"""
Example demonstrating ephemeris age anomalies detection.

This script shows how to detect stale or inconsistent Toe (Time of Ephemeris)
...
from spoof_utils import detect_ephemeris_age_anomalies, extract_satellite_timeseries_multisource, load_nav_json


def main() -> None:
    """Run ephemeris age anomalies detection example."""
    ...
    print("Ephemeris Age Anomalies Detection Example")
    print("=" * 60)
    ...
    print("Checking Toe (Time of Ephemeris) values for freshness and consistency...")
    print()
    ...
    for satellite, records in sorted(timeseries.items()):
        ...
        if findings:
            ...
            print(f"Satellite {satellite}: Found {len(findings)} ephemeris age anomalies")
            for finding in findings[:3]:  # Show first 3
                print(f"  - {finding.code} @ {finding.epoch.isoformat()}")
                print(f"    {finding.description}")
    
    if not all_findings:
        print("No ephemeris age anomalies detected.")
    else:
        print()
        print(f"Total findings: {len(all_findings)}")
        
        # Save findings
        output_file = Path("findings_ephemeris_age_anomalies.json")
